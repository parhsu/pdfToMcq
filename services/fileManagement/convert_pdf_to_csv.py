...
def convert_all_pdf_to_csv():
    pdf_data = read_pdf.read_all_pdf()

    for pdf_file in pdf_data:
        filepath = pdf_file.get('file_path')
        pdf_pages_data = pdf_file.get('file_data')
        convert_data_to_csv(filepath, pdf_pages_data)
    logging.info("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<Finished>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")


def convert_pdf_to_csv(filepath):
    pdf_pages_data = read_pdf.read_pdf(filepath)
    convert_data_to_csv(filepath, pdf_pages_data)
    logging.info("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<Finished>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")


def convert_data_to_csv(filepath,pdf_pages_data):
    csv_writer = mcq_converter()
    file_path = filepath.replace('.pdf', '.csv')
